...
web = "https://www.flipkart.com/search?q=hp%20laptops&otracker=search&otracker1=search&marketplace=FLIPKART&as-show=on&as=off"
...
min_len = min(len(titles),len(stars),len(discounts),len(prices))
titles = titles [:min_len]
stars = stars [:min_len]
discounts = discounts [:min_len]
prices = prices [:min_len]

df = pd.DataFrame({'Titles':titles,'Stars':stars,'Discounts':discounts,'Prices':prices})
print(df)

df.to_csv (r"C:\Users\Administrator\OneDrive\Documents\ApplePhones\HPlaptops.csv",index=False)