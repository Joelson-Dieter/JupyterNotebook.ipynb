...
input_element.send_keys("Mattresses"+Keys.ENTER)
time.sleep(2)
titles=[]
inches=[]
stars=[]
...
for i in range(62):
  ...
  inch=driver.find_elements(By.XPATH,"(//div[@class='NqpwHC'])")
...
min_len=min(len(titles),len(inches),len(stars),len(reviews),len(discounts),len(prices))
titles=titles[:min_len]
inches=inches[:min_len]
stars=stars[:min_len]
...
df.to_csv(r"C:\Users\Administrator\OneDrive\Documents\Projects\Mattresses.csv",index=False)